...
void tela_2(){
   ...
   tft.drawRoundRect(1,1,largMax,altMax,raio,preto);
   tft.fillRoundRect(2,2,largMax-2,altMax-2,raio, branco);
   acertaDataHora(10,5,220,110,preto,"Hora");
   acertaDataHora(10,125,220,110,amarelo, "Data");
   icone(13,245,preto); //icone rodapé
  // icone(86,245,preto); //icone rodapé
   icone(159,245,preto); //icone rodapé
...
void acertaDataHora(int16_t x0, int16_t y0,int16_t larg, int16_t alt,uint16_t cor, String txt){
  tft.drawRoundRect(x0,y0,larg,alt,raio,preto);
  tft.fillRoundRect(x0+1,y0+1,larg-2,alt-2,raio, branco);  
  texto(x0+10,y0+20,preto,1,txt);  
...
{
    tft.fillRoundRect(x, y, x+20, y+20,5, azul);
    tft.drawRoundRect(x, y, x+20, y+20,5, preto);
    for (int i=0; i<15; i++)
  tft.drawLine(x+6+(i/1.5), y+5+i, x+27-(i/1.5), y+5+i, preto);
}
